...
- 8 个领域、60 个原创学习节点、141 道带逐项解释的理解题、13 组代码挑战和 45 个行为/边界检查。
...
首页按基础能力、AI 应用、算法原理分组，保留一个继续学习入口。课程页使用单篇正文与步骤导航；先修信息默认折叠，笔记和参考资料按需打开。项目采用任务侧栏、代码区与结果面板，移动端通过任务、代码、结果切换。移动端保留底部导航。

21 节课程重写概念讲解，其余 39 节补充与本课对应的例子或计算过程，并调整简介。Python 前两节先通过修改、运行和观察理解变量与循环，第三节再引入函数挑战。基础讲解中的代码独立排版，进阶思考标为选学，不要求每节采用相同篇幅。

## 项目工作区

从左侧选择项目，再按任务逐步推进。小项目支持编辑代码、只运行、运行并检查、停止执行、查看分步提示与参考解法，以及下载代码。检查来自真实 Python / JavaScript 工作线程；修改代码后会提示结果已过期。当前运行结果不跨刷新保存。

代码、当前任务、完成标记与项目笔记自动保存到当前浏览器，导出备份时一并带走。任务标记由学习者手动核对，不会因为预设检查通过而自动完成。综合项目在自己的开发环境中运行，这里保留任务、验收条件、相关课程和记录。
...
进度、代码挑战草稿、自动保存的课程笔记、自我解释与进阶作答、小项目代码、项目记录与设置保存在当前浏览器的 `localStorage`，键为 `nicoding.learning.v1`。没有账户、云同步、统计埋点或在线 AI 调用。

在“设置 → 导出进度”备份。换浏览器、清理站点存储、更换主机名/端口前先导出。导入先验证格式、展示内容摘要，确认恢复后才替换当前记录。课程笔记、项目记录、自我解释与小项目代码均自动保存；课程笔记也保留手动保存按钮。旧版 v1 备份仍可导入；新字段缺失时初始化为空，不修改已有学习记录。
