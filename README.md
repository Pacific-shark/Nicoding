...
- 星图分为全局领域星系和局部知识星球；默认静止，在空白处按住鼠标左键拖动视角，松手即停。左侧切换层级、领域、缩放和复位，也可用方向键转动。两层独立保留本次浏览的视角。
- 局部星球展示全部领域知识及选中节点的真实先修关系，支持搜索、列表、键盘选择和定位；文字朝向屏幕，背面节点与连线淡化。点击查看详情，双击打开讲解。
