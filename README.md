...
- 全局星图展示 84 个知识点，按领域与先修关系向外分叉，形成轮廓大致圆润、远近疏密不同的立体知识群。节点分布在空间内部，连线连接真实先修关系。排布只在加载时计算，简笔猫用大小、透明度和遮挡表示深度。
- 空白处按住鼠标左键拖动，以屏幕方向转动视角；使用四元数累计旋转，翻转后横向、纵向拖动仍对应屏幕方向。默认静止、松手即停，定位知识点会将它转到正面，全局与各领域独立保留视角。
- 拖动帧只更新画布，最多保留一个待处理动画帧，失焦、切换页面时结束或清理；84 个节点复用 12 组 SVG 图形，画布不使用滤镜。桌宠只挂载当前姿势，收起时停止眨眼计时。渲染错误有知识列表和重载入口；浏览器进程内存不足无法由 React 错误边界捕获。
