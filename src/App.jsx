...
import {Roadmap,DomainPath} from './components/LearningRoadmap.jsx';
...
export default function App(){
 ...
 const petLesson=l||byId[page==='path'?paths[id].ids[0]:page==='projects'?(projects.find(p=>p.id===id)||projects[0]).skills[0]:state.last];
 ...
 useEffect(()=>{
  document.title=(page==='learn'?l.title:page==='path'?domains.find(d=>d.id===id).name:({explore:'学习路线',review:'复习',projects:'实践项目',missing:'页面未找到'}[page]))+' · Nicoding';
  window.scrollTo(0,0);
  const heading=pageRef.current?.querySelector('main h1');if(heading){heading.tabIndex=-1;heading.focus({preventScroll:true});}
  if(page==='learn')setState(s=>({...s,last:id,lessons:{...s.lessons,[id]:{...s.lessons[id],visited:s.lessons[id]?.visited||Date.now()}}}));
 },[page,id,view,setState]);
 useEffect(()=>{const key=e=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();setQuery('');setModal('search');}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key);},[]);
 ...
 return <div ref={pageRef} className={`app page-${page}`}>
  <header className="app-header"><a className="brand" aria-label="Nicoding 首页" href="#explore"><img src="./assets/nicoding-logo.png" alt="Nicoding"/></a><nav className="top-nav" aria-label="主导航">{[['explore','学习路线'],['learn','继续学习'],['review','复习'],['projects','项目']].map(([target,label])=><a key={target} href={'#'+target+(target==='learn'?'/'+state.last:'')} className={page===target||target==='explore'&&page==='path'?'active':''} aria-current={page===target||target==='explore'&&page==='path'?'page':undefined}>{label}{target==='review'&&lessons.some(x=>state.lessons[x.id]?.completed&&state.lessons[x.id].due<=Date.now())&&<i/>}</a>)}</nav><div className="header-actions"><button className="search-trigger" onClick={()=>{setQuery('');setModal('search');}}><Icon name="Search" size={17}/><span>搜索知识点</span><kbd>Ctrl K</kbd></button><IconButton name="Settings2" label="设置与进度备份" onClick={()=>setModal('settings')}/></div></header>
  ...
  {page==='explore'&&<Roadmap state={state}/>}
  {page==='path'&&<DomainPath key={id} id={id} state={state}/>}
  {page==='learn'&&<Lesson key={id} lesson={l} startStep={view==='explain'?4:0} progress={progress} onBack={()=>navigate('path',l.domain)} onSelect={learn} onCelebrate={()=>setCelebration(v=>v+1)}/>}
  ...
  {state.settings.pet&&<Pet lesson={petLesson} completed={lessons.filter(l=>state.lessons[l.id]?.completed).length} suspended={!!modal} onLearn={learn} onTeach={id=>navigate('learn',id+'/explain')} onProject={id=>navigate('projects',id)} motion={state.settings.motion} celebration={celebration} resetKey={petReset}/>}
