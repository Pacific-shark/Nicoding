...
export default function App(){
 ...
 useEffect(()=>{
  document.title=(page==='course'?(unitById[view]?.title||'第一份分类模型'):page==='chapter'?(id==='ml'?'这一轮，联系谁？':chapters[id].name):page==='journey'?journeys[id].title:page==='learn'?l.title:page==='path'?domains.find(d=>d.id===id).name:({explore:'Nico 杂货铺',library:'知识索引',review:'复习',projects:'实践项目',missing:'页面未找到'}[page]))+' · Nicoding';
  window.scrollTo(0,0);
  ...
  if(page==='learn')setState(s=>({...s,last:id,lastJourney:null,lastCourse:null,lessons:{...s.lessons,[id]:{...s.lessons[id],visited:s.lessons[id]?.visited||Date.now()}}}));
 ...
 return <div ref={pageRef} className={`app page-${page}`}>
  ...
  {page==='chapter'&&id!=='ml'&&<ChapterOpening key={id} id={id} state={state}/>}
  {(page==='course'||page==='chapter'&&id==='ml')&&<Suspense fallback={<main role="status" className="collection-page">正在打开机器学习项目…</main>}><BankCourse key={page==='course'?view:'opening'} unitId={page==='course'?view:undefined} progress={progress}/></Suspense>}
