...
export default function App(){
 ...
 const petLesson=l||byId[page==='journey'?journeys[id].lessons[Number(view)||0]:page==='path'?paths[id].ids[0]:page==='projects'?(projects.find(p=>p.id===id)||projects[0]).skills[0]:state.last];
 ...
 useEffect(()=>{
  document.title=(page==='journey'?journeys[id].title:page==='learn'?l.title:page==='path'?domains.find(d=>d.id===id).name:({explore:'学习路线',review:'复习',projects:'实践项目',missing:'页面未找到'}[page]))+' · Nicoding';
  window.scrollTo(0,0);
 ...
 return <div ref={pageRef} className={`app page-${page}`}>
  <header className="app-header"><a className="brand" aria-label="Nicoding 首页" href="#explore"><img src="./assets/nicoding-logo.png" alt="Nicoding"/></a><nav className="top-nav" aria-label="主导航">{[['explore','学习路线'],['learn','继续学习'],['review','复习'],['projects','项目']].map(([target,label])=><a key={target} href={target==='learn'&&state.lastJourney?'#journey/'+state.lastJourney:'#'+target+(target==='learn'?'/'+state.last:'')} className={page===target||target==='learn'&&page==='journey'||target==='explore'&&page==='path'?'active':''} aria-current={page===target||target==='learn'&&page==='journey'||target==='explore'&&page==='path'?'page':undefined}>{label}{target==='review'&&lessons.some(x=>state.lessons[x.id]?.completed&&state.lessons[x.id].due<=Date.now())&&<i/>}</a>)}</nav><div className="header-actions"><button className="search-trigger" aria-label="搜索知识点（Ctrl K）" onClick={()=>{setQuery('');setModal('search');}}><Icon name="Search" size={17}/><span>搜索知识点</span><kbd>Ctrl K</kbd></button><IconButton name="Settings2" label="设置与进度备份" onClick={()=>setModal('settings')}/></div></header>
