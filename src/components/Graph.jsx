...
export default function Graph({selected,onSelect,domain,setDomain,state}) {
 const host=useRef(),drag=useRef(),pointer=useRef({x:0,y:0});
 const [size,setSize]=useState({w:850,h:780}),[view,setView]=useState('space'),[priority,setPriority]=useState(false),[zoom,setZoom]=useState(1),[pan,setPan]=useState({x:0,y:0}),[tilt,setTilt]=useState({x:0,y:0});
 ...
 const root=nodes.find(n=>n.lesson.id===selected)||nodes[0];
 ...
 function reset(){setPan({x:0,y:0});setZoom(1);setTilt({x:0,y:0});}
 useEffect(reset,[domain,view]);
 function down(e){if(e.button!==0||e.target.closest('button'))return;drag.current={x:e.clientX,y:e.clientY,pan};e.currentTarget.setPointerCapture(e.pointerId);}
 function move(e){
  if(drag.current){setPan({x:drag.current.pan.x+e.clientX-drag.current.x,y:drag.current.pan.y+e.clientY-drag.current.y});}
  else if(view==='space'&&state.settings.motion&&!window.matchMedia('(prefers-reduced-motion: reduce)').matches){const r=e.currentTarget.getBoundingClientRect();pointer.current={x:e.clientX-r.left,y:e.clientY-r.top};setTilt({x:(pointer.current.x/r.width-.5)*9,y:(pointer.current.y/r.height-.5)*7});}
 }
 return <section className={`graph-panel ${view==='flat'?'flat':''}`} aria-label="交互知识树">
  <div className="graph-toolbar"><label className="select-wrap"><span className="sr-only">知识领域</span><select aria-label="知识领域" value={domain} onChange={e=>setDomain(e.target.value)}><option value="all">全部领域</option>{domains.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select><Icon name="ChevronDown" size={14}/></label><div className="segmented"><button className={view==='space'?'active':''} onClick={()=>setView('space')}>空间</button><button className={view==='flat'?'active':''} onClick={()=>setView('flat')}>平面</button></div><button className="toggle-label" onClick={()=>setPriority(!priority)} role="switch" aria-checked={priority}><span className={`switch ${priority?'on':''}`}/><span>只看优先</span></button></div>
  <div className="graph-stage" ref={host} onPointerDown={down} onPointerMove={move} onPointerUp={()=>drag.current=null} onPointerCancel={()=>drag.current=null} onPointerLeave={()=>{if(!drag.current)setTilt({x:0,y:0});}} onWheel={e=>{if(e.ctrlKey||Math.abs(e.deltaY)>0)setZoom(z=>Math.min(1.75,Math.max(.65,z-e.deltaY*.0006)));}}>
   {nodes.length===0?<div className="graph-empty"><Icon name="Sparkles" size={32}/><h3>这个领域可以按顺序慢慢来</h3><p>目前没有标为优先的节点。</p><button onClick={()=>setPriority(false)} className="text-button">显示全部节点 <Icon name="ArrowRight"/></button></div>:<div className="graph-world" style={{width:900,height:810,transform:`translate(-50%,-50%) translate(${pan.x+tilt.x}px,${pan.y+tilt.y}px) scale(${scale})`}}>
