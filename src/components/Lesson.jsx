...
import {Icon,SourceLinks,statusLabel,statusOf} from './ui.jsx';
...
function Editor({lesson,challengeMode,onEvidence,onAssist,savedCode,onCodeChange}) {
...
export default function Lesson({lesson,progress,onBack,onSelect,onCelebrate}) {
 ...
 const steps=['理解概念','看懂示例',lesson.challenge?'动手练习':'改变与观察','检查理解'];
 ...
 return <main className="lesson-layout">
  <div className="lesson-heading"><button className="back-link" onClick={onBack}><Icon name="ArrowLeft"/>返回知识树</button><div><div className="breadcrumb">{domain.name}<span>/</span>{lesson.title}</div><h1>{lesson.title}</h1><p>{lesson.subtitle}</p></div><span className="lesson-estimate"><Icon name="Clock" size={15}/>{lesson.minutes} 分钟起 · {lesson.depth}</span></div>
  <aside className="lesson-steps surface"><h3>学习步骤</h3><nav aria-label="学习步骤">{steps.map((s,i)=><button key={s} className={i===step?'active':''} aria-current={i===step?'step':undefined} onClick={()=>setStep(i)}><span>{String(i+1).padStart(2,'0')}</span>{s}{(i===3&&quizPassed||i===2&&evidence)&&<Icon name="Check" size={14}/>}</button>)}</nav><div className="aside-section"><h3>本关目标</h3><ol>{lesson.parts.slice(0,3).map(([title],i)=><li key={title}><span>{i+1}</span>{title}</li>)}</ol></div><div className="aside-section"><h3>先修知识</h3>{lesson.prereqs.length?lesson.prereqs.map(id=><button className="prereq-link" key={id} onClick={()=>onSelect(id)}><Icon name={state.lessons[id]?.completed?'CheckCircle2':'BookOpen'} size={15}/>{byId[id].title}<Icon name="ChevronRight" size={14}/></button>):<p className="muted">从这里开始就好，无需编程前提。</p>}</div><p className="sidebar-footnote">慢一点，把原因想清楚。<br/>学习时间是建议，不是倒计时。</p></aside>
  <section className="lesson-center surface" aria-label="课程内容">
